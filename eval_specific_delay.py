...
def main():
    parser = argparse.ArgumentParser(description="Evaluate specific Delay models for each scenario and policy combination.")
    parser.add_argument("--data-dir", default="Data_cleaned", help="Data directory (e.g. Data_cleaned or Data)")
    parser.add_argument("--checkpoint-dir", default="checkpoints_v2", help="Checkpoints directory containing SC01_PF, etc.")
    ...
    for sc_id, cfgs in groups.items():
        ...
        # Group configs by scheduling policy
        policy_groups = {}
        for c in delay_cfgs:
        ...
        for policy, p_cfgs in policy_groups.items():
            ...
            if not os.path.exists(ckpt_path):
                # Optionally, print that it was skipped if you want to see what's missing
            ...
            data_paths = [c.data_path for c in p_cfgs]
