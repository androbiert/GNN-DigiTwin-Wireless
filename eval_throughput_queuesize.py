...
def parse_qsize_for_sort(q):
    """Parse queue size string into a numeric value (in KiB) for sorting."""
    try:
...
def main():
    ...
    for sc_id, cfgs in groups.items():
        ...
        # Sort queue sizes numerically
        sorted_qsizes = sorted(qsize_folders.keys(), key=parse_qsize_for_sort)

        # Evaluate on each queue size
        for qsize in sorted_qsizes:
            folders = qsize_folders[qsize]
            # Filter the test dataset for this queue size
            qsize_graphs = [g for g in full_test_ds.graphs if g["config_folder"] in folders]
            
            if not qsize_graphs:
                print(f"  [{qsize}] No test graphs found.")
                continue

            print(f"  [{qsize}] Test graphs: {len(qsize_graphs)}")
            
            # Predict
            all_pred = []
            all_true = []
            
            pol_test_ds = WirelessDataset(qsize_graphs, normalizer=normalizer)
            ...
            if all_pred:
                ...
                print(f"    [Sample Comparison for {qsize}]")
    ...
    print(f"{'='*70}")
    print(f"{'Scenario':<10} {'Queue Size':<12} {'MAE (kbps)':>12} {'RMSE (kbps)':>12} {'MAPE (%)':>10} {'R²':>10}")
    print("-" * 70)
    for r in results:
        print(f"{r['Scenario']:<10} {r['Queue Size']:<12} {r['MAE']*1e-3:>12.2f} {r['RMSE']*1e-3:>12.2f} {r['MAPE']:>10.2f} {r['R2']:>10.4f}")
