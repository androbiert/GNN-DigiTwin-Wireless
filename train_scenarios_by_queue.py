"""
train_scenarios_by_queue.py — Per-Scenario × Queue-Size Training

Trains one GNN model per (Scenario × Queue Size × Target) combination.
Within each model, ALL power levels and schedulers are pooled together.

Queue sizes: 50KiB, 100KiB, 2MiB, 10MiB

Usage:
  # Train all scenarios for delay, split by queue size
  python train_scenarios_by_queue.py --target delay --epochs 50

  # Train single scenario for both targets
  python train_scenarios_by_queue.py --scenario SC01 --target all --epochs 50

  # Train with resume
  python train_scenarios_by_queue.py --target delay --resume --epochs 100

Checkpoints:
  checkpoints_queue/<scenario>_<queue_size>/<target>/best.pt
  checkpoints_queue/<scenario>_<queue_size>/<target>/epoch_NNN_mape_XXXX.pt
"""
...
# --------------------------------------------------------------------------- #
# Train one scenario × queue_size × target
# --------------------------------------------------------------------------- #

def train_scenario_queue(
    scenario_id:    str,
    queue_size:     str,
    target:         str,
    data_paths:     List[str],
    ...
    patience:       int   = 10,
    device_str:     str   = "auto",
    checkpoint_dir: str   = "checkpoints_queue",
    seed:           int   = 42,
    ...
    """
    label = f"{scenario_id}_{queue_size}/{target.capitalize()}"
...
def plot_loss_curve(history: dict, scenario_id: str, queue_size: str, target: str, save_dir: str):
    ...
    ax.set_title(f"{scenario_id} (Q={queue_size}) — Training Curve ({target.capitalize()})",
                 fontsize=14, fontweight="bold")
    ax.legend(fontsize=12)
    ax.grid(True, alpha=0.3)
    ax.set_yscale("log")
    path = os.path.join(save_dir, f"loss_curve_{scenario_id}_{queue_size}_{target}.png")
    fig.savefig(path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"[{scenario_id}_{queue_size}] Loss curve saved -> {path}")
...
def plot_scatter(results_list: list, scenario_id: str, queue_size: str, target: str,
    ...
    ax.set_title(f"{scenario_id} (Q={queue_size}) — Prediction Scatter ({target.capitalize()})",
                 fontsize=13, fontweight="bold")
    ax.legend(fontsize=11)
    ax.grid(True, alpha=0.3)

    path = os.path.join(save_dir, f"scatter_{scenario_id}_{queue_size}_{target}.png")
    fig.savefig(path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"[{scenario_id}_{queue_size}] Scatter plot saved -> {path}")
...
def print_results_table(all_results: List[dict]):
    ...
    # Separate delay and throughput results since they use different metrics
    ...
    if delay_results:
        ...
        scale = 1000.0
        print(f"\n{'='*100}")
        print(f"  TRAINING RESULTS SUMMARY — DELAY (GROUPED BY QUEUE SIZE)")
        print(f"  Loss = Log-Huber (what the model optimizes)  |  MAE = physical error in {unit}")
        print(f"{'='*100}")
        print(f"  {'Scenario':<10} {'Queue':<10} {'Train Loss':>12} {'Val Loss':>12} {'Test Loss':>12} {'Test MAE':>12}")
        print(f"  {'-'*70}")
        for r in sorted(delay_results, key=lambda x: (x["scenario"], x["queue_size"])):
            train_final = r["history"]["train"][-1] if r["history"]["train"] else 0
            test_mae_val = r.get("test_mae", 0) * scale
            print(
                f"  {r['scenario']:<10} {r['queue_size']:<10} "
    ...
    if tput_results:
        ...
        scale = 1e-3
        print(f"\n{'='*100}")
        print(f"  TRAINING RESULTS SUMMARY — THROUGHPUT (GROUPED BY QUEUE SIZE)")
        print(f"{'='*100}")
        print(f"  {'Scenario':<10} {'Queue':<10} {'Train MAPE':>12} {'Val MAPE':>12} {'Test MAPE':>12} {'Test MAE':>12}")
        print(f"  {'-'*70}")
        for r in sorted(tput_results, key=lambda x: (x["scenario"], x["queue_size"])):
            train_final = r["history"]["train"][-1] if r["history"]["train"] else 0
            test_mae_val = r.get("test_mae", 0) * scale
            print(
                f"  {r['scenario']:<10} {r['queue_size']:<10} "
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Train WirelessNet-Fermi per scenario × queue_size × target",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python train_scenarios_by_queue.py --target delay --epochs 50
  python train_scenarios_by_queue.py --target all --epochs 50
  python train_scenarios_by_queue.py --scenario SC01 --target delay --epochs 30
        """,
    )
    ...
    parser.add_argument("--queue-size", default=None,
                        help="Train only this queue size (e.g. 50KiB, 100KiB, 2MiB, 10MiB). Default: all.")
    ...
    parser.add_argument("--checkpoint-dir", default="checkpoints_queue")
    ...
    # Build training plan: group by (scenario, queue_size)
    training_plan = []  # list of (model_name, scenario_id, queue_size, target, data_paths)
    for sc_id, cfgs in groups.items():
        for tgt in targets:
            ...
            for qsize, q_cfgs in sorted(queue_groups.items()):
                model_name = f"{sc_id}_{qsize}"
                paths = [c.data_path for c in q_cfgs]
                training_plan.append((model_name, sc_id, qsize, tgt, paths))

    # Filter by queue size if specified
    if args.queue_size:
        original_len = len(training_plan)
        training_plan = [tp for tp in training_plan if tp[2] == args.queue_size]
        print(f"\n[FILTER] Filtered to queue_size={args.queue_size}: {len(training_plan)}/{original_len} models")

    print(f"\n{'='*70}")
    print(f"  PHASE 2: TRAINING PLAN")
    print(f"{'='*70}")
    for model_name, sc_id, qsize, tgt, paths in training_plan:
        print(f"  {model_name:<20} × {tgt:<12} → {len(paths):>2} data files")
    print(f"\n  Total models to train: {len(training_plan)}")
    print(f"{'='*70}\n")

    if not training_plan:
        print("Nothing to train. Check your --scenario / --target / --queue-size filters.")
        sys.exit(1)

    # ── Step 3: Train each ────────────────────────────────────────────────── #
    all_results = []

    for i, (model_name, sc_id, qsize, tgt, paths) in enumerate(training_plan):
        print(f"\n{'#'*70}")
        print(f"  MODEL {i+1}/{len(training_plan)}: {model_name} × {tgt}")
        print(f"{'#'*70}")
        ...
        try:
            ...
            plot_loss_curve(result["history"], sc_id, qsize, tgt, plot_dir)
            ...
            if test_results:
                plot_scatter(test_results, sc_id, qsize, tgt, plot_dir)

            all_results.append(result)

        except Exception as e:
            print(f"\n[ERROR] {model_name}/{tgt} failed: {e}")
            import traceback
            traceback.print_exc()
            continue
