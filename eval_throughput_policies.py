    ...
    for sc_id, cfgs in groups.items():
        ...
        for policy, folders in policy_folders.items():
            ...
            if all_pred:
                ...
                scale = 1e-3
                print(f"    -> MAE: {metrics['MAE'] * scale:.2f} kbps | RMSE: {metrics['RMSE'] * scale:.2f} kbps | MAPE: {metrics['MAPE (%)']:.2f}% | SMAPE: {metrics['SMAPE (%)']:.2f}% | R²: {metrics['R²']:.4f} | Acc@10: {metrics['Acc@10%']:.1f}% | Acc@10: {metrics['Acc@20%']:.1f}% | Time: {avg_infer_ms:.2f} ms/graph")
                
                print(f"    [Sample Comparison for {policy} (Rel Err <= 15%)]")
